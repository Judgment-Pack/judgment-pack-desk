...
| Medium | Navigation, surfaces, text, selection and status used an accent that did not match the selected reference. | Dark sidebar, neutral frame/cards and muted inactive navigation. The later [pack workspace color audit](reviews/pack-workspace-color-audit.md) replaces indigo actions with Judgment Pack green and gold. Link/focus and action-fill tokens are separate so white labels remain readable. |
