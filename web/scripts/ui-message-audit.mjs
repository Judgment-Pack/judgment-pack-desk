...
const props = new Set(['label', 'title', 'placeholder', 'aria-label', 'aria-description', 'aria-valuetext', 'ariaLabel', 'expectedLabel', 'expected', 'actualLabel', 'empty', 'emptyWhat', 'context', 'mapUnavailable', 'fallback', 'description', 'action'])
const exact = new Set(['JSON', 'JPS', 'MCP', 'RFC', 'README', 'sha256', 'v', 'id', 'null', 'true', 'false', 'Google Gemini', 'Anthropic'])
export function untranslatedUiMessages(text) {
  ...
  function walk(node, ancestors = []) {
    const inMessage = ancestors.some(p => p.type === 'CallExpression' && ['msg', 'sourceMessage', 'systemMessage'].includes(p.callee.name))
      || ancestors.some(p => p.type === 'JSXOpeningElement' && p.name.name === 'Message')
    if (!inMessage && ['StringLiteral', 'JSXText', 'TemplateLiteral'].includes(node.type)) {
      ...
      if ((node.type === 'JSXText' || isProp || isOption || isChild || confirmation) && /[A-Za-z]/.test(value.replaceAll('{{value}}', '')) && !exact.has(value)) {
