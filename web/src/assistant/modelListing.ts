 ...
 * on a refusal and is not quoted on any path. The probe's own ruling is the
 * reason: a body under the endpoint's control can carry a derived
 * representation of the credential that no substitution reliably finds.
 */
...
export function listingRefusal(status: number): string {
  return `the model listing was refused — answered ${status}, ${DIAGNOSTIC_SAYS[diagnosticFor(status)]}`
}
