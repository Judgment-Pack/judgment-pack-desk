...
function setup(options:{connected?:boolean;enabled?:boolean;modelError?:boolean;model?:string|null;slotOnly?:boolean;tools?:string[];runtimeMissing?:boolean;holdLogin?:boolean;failLoginOnce?:boolean;alreadyConnected?:boolean}={}) {
  ...
  vi.stubGlobal('fetch',async(input:string,init:RequestInit={})=>{
    ...
    if(path==='/api/model-providers')value={providers:[{id:'openai',authMethod:'subscription',agent:'codex',configured:options.enabled!==false,enabled:options.enabled!==false,engineReady:options.enabled!==false,requiredVersion:'0.145.0',availability:options.enabled===false?'disabled':'available',loginMethods:['browser','device']}]}
