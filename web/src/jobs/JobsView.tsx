...
export function CreateJobContent() {
  ...
  const [inputMode, setInputMode] = useState<'manual' | InputMapping['provider']>('manual'), [source, setSource] = useState<SourceInput>()
  ...
  async function prepare() {
    ...
    try {
      ...
      if (source && inputMode !== 'manual') await verifySource(source, research.gateway)
  ...
  return <>
    ...
    <PageBody width="form"><div className={styles.stack}>
      ...
      <section className={styles.stack}><h2>{msg('Sample inputs')}</h2>
        ...
        {inputMode === 'manual' ? <InputFields {...{ facts, setFacts, supplied, setSupplied, evidence, setEvidence }} disabled={busy} /> : pack.data && <SourceInputFields key={`${packId}:${inputMode}`} doc={pack.data.document} provider={inputMode} disabled={busy} onChange={setSource} />}
...
function JobView({ jobId }: { jobId: string }) {
  ...
  return <>
    ...
    <PageBody width="wide"><div className={styles.stack}><Problem error={query.error || runs.error} />
      ...
      {data && <><div className={styles.release}><h2>{data.release.title}</h2><p className="quiet">{msg('Fixed version {{version}} · Manual / API', { version: data.release.packVersion })}</p>{data.release.inputMapping && <p className={styles.note}>{data.release.inputMapping.provider === 'local-file' ? msg('Local JSON file · Fixed input mapping') : msg('Google Drive · Fixed input mapping')}</p>}<ReleaseReadiness release={data.release} /><Disclosure title={msg('Release details')}><dl className={styles.properties}><div><dt>{msg('Pack digest')}</dt><dd><code>{data.release.packDigest}</code></dd></div><div><dt>{msg('Runtime digest')}</dt><dd><code>{data.release.runtimeDigest}</code></dd></div></dl><p className={styles.note}>{msg('To use a changed pack, create a new job and review its new release.')}</p></Disclosure></div>
...
function RunForm({ job, release }: { job: Job; release: Release }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (source) await verifySource(source, research.gateway)
  ...
  return <section className={styles.runForm}><h2>{msg('New run')}</h2><p className="quiet">{msg('These inputs start an operational run and will be retained with its audit record.')}</p>{release.inputMapping ? <SourceInputFields doc={JSON.parse(release.pack) as PackDocument} provider={release.inputMapping.provider} fixed={release.inputMapping} disabled={busy} onChange={setSource} /> : <InputFields {...{ facts, setFacts, supplied, setSupplied, evidence, setEvidence }} disabled={busy} />}<Problem error={error} /><div><Button variant="primary" disabled={busy || Boolean(release.inputMapping && !source)} onClick={() => { void submit() }}>{busy ? msg('Submitting…') : msg('Submit run')}</Button></div></section>
}
  ...
  return <>
    ...
    <PageBody width="wide"><div className={styles.stack}><Problem error={query.error} />
      ...
      {run && <><dl className={styles.properties}><div><dt>{msg('Execution')}</dt><dd role={run.state === 'queued' || run.state === 'running' ? 'status' : undefined}>{stateLabel(run.state)}</dd></div><div><dt>{msg('Decision')}</dt><dd>{decisionLabel(run.result)}</dd></div><div><dt>{msg('Submitted')}</dt><dd>{date(run.createdAt)}</dd></div><div><dt>{msg('Finished')}</dt><dd>{run.finishedAt ? date(run.finishedAt) : '—'}</dd></div></dl>
        ...
        {run.input?.source ? <SourceSummary source={run.input.source} /> : <JSONView title={msg('Retained inputs')} value={run.input} />}
