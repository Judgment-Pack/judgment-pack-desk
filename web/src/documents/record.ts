...
export interface DocumentRecord {
  ...
  provenance: { adapter: { name: string; version: string; digest: string }; source: { kind: 'inline'|'google-drive'|'gmail'|'connected-source'|'web'|'connection-resource'; provider?: string; resourceId?: string; url?: string; requestedUrl?: string; responseDigest?: string; messageId?: string; threadId?: string; format?: string; fileId?: string; version?: string; mediaType?: string }; observedAt: string; processor: string | null; ocr: { program: string; digest: string; pages: number[] } | null }
}
...
export function readDocumentRecord(value: unknown): DocumentRecord {
  ...
  require(['google-drive','gmail','connected-source','web','connection-resource'].includes(sourceKind as string) ? o.retention === 'inline' && o.encoding === 'base64' && str(o.bytes) : o.retention === 'caller' && o.encoding === null && o.bytes === null)
  ...
  require(str(adapter.name) && adapter.name.length > 0 && str(adapter.version) && digest(adapter.digest) && ['inline', 'google-drive', 'gmail', 'connected-source', 'web', 'connection-resource'].includes(source.kind as string))
  ...
  if (source.kind === 'web') require(validWebURL(source.requestedUrl) && validWebURL(source.url) && digest(source.responseDigest) && source.version === d.version && source.version === d.id && v.ocr === null && (source.format === 'static-text-v1' ? source.mediaType === 'text/html' && d.mediaType === 'text/plain' && v.processor === 'adapter-document/text/1' : source.format === 'original-v1' && ['text/plain','application/pdf'].includes(source.mediaType as string) && source.mediaType === d.mediaType && source.responseDigest === d.id))
  if (source.kind === 'connection-resource') require(str(source.provider) && /^[a-z][a-z0-9-]{0,47}$/.test(source.provider) && str(source.resourceId) && enc.encode(source.resourceId).length >= 1 && enc.encode(source.resourceId).length <= 4096 && !/[\x00-\x1f\x7f]/.test(source.resourceId) && (source.url === '' || validWebURL(source.url) && !source.url.includes('?')) && source.version === d.version && source.version === d.id && source.format === 'retained-file-v1')
  if (source.kind === 'connected-source') require(validConnectedSource(source) && source.version === d.version && source.version === d.id && source.format === 'text-snapshot-v1' && d.mediaType === 'text/plain' && v.processor === 'adapter-document/text/1')
...
function validConnectedSource(s: Record<string, unknown>): boolean {
 if (typeof s.resourceId !== 'string' || typeof s.url !== 'string' || enc.encode(s.url).length > 4096) return false
 try {
  const url = new URL(s.url), id = s.resourceId
  if (url.username || url.password || url.hash) return false
  if (s.provider === 'notion') return /^(?:[a-f0-9]{32}|[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})$/.test(id) && url.protocol === 'https:' && !url.port && ['notion.so','www.notion.so','notion.com','www.notion.com'].includes(url.hostname) && url.pathname.replaceAll('-', '').endsWith(id.replaceAll('-', ''))
  if (s.provider === 'obsidian') return id.length > 0 && enc.encode(id).length <= 1024 && !/[\\\x00-\x1f\x7f:]/.test(id) && /\.md$/i.test(id) && id.split('/').every(part => part && !part.startsWith('.')) && url.protocol === 'obsidian:' && url.host === 'open' && !url.pathname && [...url.searchParams].length === 2 && url.searchParams.getAll('vault').length === 1 && Boolean(url.searchParams.get('vault')) && url.searchParams.getAll('file').length === 1 && url.searchParams.get('file') === id.replace(/\.[^.]+$/, '')
 } catch { return false }
 return false
}
