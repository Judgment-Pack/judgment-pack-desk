...
export async function signedDocument(drive = false) {
  ...
  if (drive) {
    ...
    Object.assign(record.provenance.source, {kind: 'google-drive', fileId: 'file-A', version: '7', mediaType: 'text/plain'})
  }
  ...
  const salt = new Uint8Array(32).fill(7), args = canonicalText(JSON.stringify(drive ? driveRequest : documentArguments(original)))
  ...
  const object: DocumentObject = { version:1, original, proof: { session:'doc-test', source:'documents', authority:gw.authority, publicKey:TEST_PUBLIC_KEY, response:JSON.stringify({ result:record, receipt:signed, salts:{args:'07'.repeat(32)} }), registry:await gw.registry(), ...(drive ? {drive: driveRequest} : {}) } }
