...
export async function signedDocument(drive: boolean | 'gmail' = false) {
  ...
  const response = await gw.acquire('doc-test',drive === 'gmail' ? 'gmail' : 'documents',record)
  ...
  const salt = new Uint8Array(32).fill(7), args = canonicalText(JSON.stringify(drive === 'gmail' ? mailRequest : drive ? driveRequest : documentArguments(original)))
  ...
  const object: DocumentObject = { version:1, original, proof: { session:'doc-test', source:drive === 'gmail' ? 'gmail' : 'documents', authority:gw.authority, publicKey:TEST_PUBLIC_KEY, response:JSON.stringify({ result:record, receipt:signed, salts:{args:'07'.repeat(32)} }), registry:await gw.registry(), ...(drive === 'gmail' ? {gmail: mailRequest} : drive ? {drive: driveRequest} : {}) } }
