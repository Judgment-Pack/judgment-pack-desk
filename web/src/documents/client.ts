...
export interface DocumentObject {
  ...
  proof?: { session: string; source: string; authority: string; publicKey: string; response: string; registry: string; drive?: DriveSelection; gmail?: MailSelection }
}
...
export async function verifyDocument(object: DocumentObject, pin: ResearchGatewayConfig, expectedDigest?: string): Promise<VerifiedDocument> {
  ...
  if (proof?.drive && proof?.gmail) throw fail()
  ...
  if (!acquisition || stringMember(acquisition, 'shape') !== (proof.drive || proof.gmail ? 'http' : 'command')) throw fail()
  ...
  const args = canonicalize(parseJsonText(JSON.stringify(proof.drive ?? proof.gmail ?? documentArguments(original))))
  ...
  if (!proof.drive && !proof.gmail && record.provenance.source.kind !== 'inline') throw fail()
...
async function ingestSelected(selection: DriveSelection | MailSelection, source: 'drive' | 'gmail', config: ResearchConfig, signal: AbortSignal, progress: (message: string) => void): Promise<{ reference: DocumentReference; document: VerifiedDocument }> {
 ...
 if (record.provenance.source.kind !== (source === 'gmail' ? 'gmail' : 'google-drive') || record.original.retention !== 'inline' || !record.original.bytes) throw fail()
 ...
 object.proof = {session, source, authority: gateway.authority, publicKey: gateway.signer.public, response: response.text, registry: registryText, ...(source === 'gmail' ? {gmail: selection as MailSelection} : {drive: selection as DriveSelection})}
 await save(id, object, stored, signal)
 const document = await verifyDocument(object, gateway)
 signal.throwIfAborted()
