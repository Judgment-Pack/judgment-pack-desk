...
describe('map group addresses', () => {
  ...
  it('restores a group deep link, then replaces it with the chosen real member pointer', async () => {
    ...
    fireEvent.click(within(screen.getByRole('heading', { name: 'Result handling · 2' }).parentElement!).getByRole('button', { name: 'View details: Handoff settings' }))
...
describe('the guided reading workspace', () => {
  ...
  it('opens on an overview and keeps detailed members in their named sections', async () => {
    ...
    expect(screen.getByRole('region', { name: 'Source references' })).toBeTruthy()
