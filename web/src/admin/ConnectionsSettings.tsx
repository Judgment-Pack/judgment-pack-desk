...
import { decodeDeskConfig, DESK_DEFAULTS, DOCUMENT_DEFAULTS, DOCUMENT_LIMIT_BOUNDS, type ConfigProblem, type EffectiveConfig, type ResearchConfig } from '../config/deskConfig'
...
function snapshot(effective: EffectiveConfig): Snapshot {
  return { research: effective.config.research, digest: effective.desk?.readFailure || effective.desk?.problems.length ? undefined : effective.desk?.sha256, text: effective.desk?.text }
}
function editable(research: ResearchConfig) {
  return { gateway: research.gateway ?? blankGateway, documents: research.documents ?? { ...DOCUMENT_DEFAULTS, enabled: false } }
}
...
export function ConnectionsSettings() {
  ...
  const [draft, setDraft] = useState(() => editable(current.research))
  ...
  const dirty = editor !== null && JSON.stringify(draft) !== JSON.stringify(editable(base.research))
  useUnsavedChanges(dirty)
  useEffect(() => { setCurrent(snapshot(effective)) }, [effective])

  function open(kind: Editor) {
    setBase(current); setDraft(editable(current.research)); setEditor(kind)
  ...
  function reflect(next: Snapshot, path: string) {
    setCurrent(next)
    client.setQueryData<EffectiveConfig>(DESK_CONFIG_QUERY_KEY, previous => previous && ({
      ...previous, config: { ...previous.config, research: next.research },
      desk: { ...previous.desk, present: Boolean(next.text), path, sha256: next.digest, text: next.text, decoded: next.text ? decodeDeskConfig(next.text, 'desk') : undefined, problems: [], readFailure: undefined }
    }))
  }
  async function reload() {
    ...
    try {
      ...
      reflect(next, read.path); setBase(next); setDraft(editable(next.research)); setError(''); setProblems([])
    } catch (cause) { setError((cause as Error).message) }
    finally { setBusy(false) }
  }
  async function save() {
    if (!editor || !dirty || busy || base.digest === undefined) return
    const research = { ...base.research, ...(editor === 'gateway' ? { gateway: draft.gateway } : { documents: draft.documents }) }
    ...
    if (draft.documents.enabled && !research.gateway) { setError(sourceMessage('Set up the gateway before enabling PDF processing.')); return }
  ...
  const gateway = current.research.gateway, documents = current.research.documents
  ...
  return <>
    <SettingsSection title={msg('Connections')} level={2} variant="standalone" description={msg('Connect sources for chats and research.')}>
      ...
      <SettingRow title={msg('Gateway')} description={msg('Shared by research and PDF processing.')}
        status={gateway ? msg('Configured · Availability checked when used') : msg('Not configured')}
        action={<Button ref={gatewayOpener} disabled={!writable} aria-label={gateway ? msg('Manage gateway') : msg('Set up gateway')} onClick={() => open('gateway')}>{gateway ? msg('Manage') : msg('Set up')}</Button>} />
    ...
    <Dialog open={editor !== null} onOpenChange={isOpen => { if (!isOpen) close() }} openerRef={opener}
      ...
      {!discard && <form id={formId} noValidate onSubmit={event => { event.preventDefault(); void save() }}><FieldGroup>
        {editor === 'gateway' ? <>
          ...
          <Disclosure title={msg('Advanced settings')} open={advanced} onToggle={event => setAdvanced(event.currentTarget.open)}><FieldGroup>
            <Field label={msg('Document source name')} error={fieldError('research.documents.source')}>{w => <Input {...w} value={draft.documents.source} disabled={busy} onChange={event => change({ documents: { ...draft.documents, source: event.target.value } })} />}</Field>
            {(['maxRequestBytes', 'maxResponseBytes'] as const).map(key => <Field key={key} label={key === 'maxRequestBytes' ? msg('Gateway request limit (MiB)') : msg('Extraction response limit (MiB)')} error={fieldError(`research.documents.${key}`)}>{w => <Input {...w} type="number" step="any" min={DOCUMENT_LIMIT_BOUNDS[key][0] / MIB} max={DOCUMENT_LIMIT_BOUNDS[key][1] / MIB} value={draft.documents[key] / MIB} disabled={busy} onChange={event => change({ documents: { ...draft.documents, [key]: Number(event.target.value) * MIB } })} />}</Field>)}
