...
import { GoogleRegistrationGuide } from '../connections/GoogleRegistrationGuide'
...
function Registrations({ available }: { available: boolean }) {
  ...
  const [selection, setSelection] = useState<{ provider: ConnectionProvider; guide: boolean } | null>(null)
  ...
  const reset = useCallback(() => setWidth(420), [])
  const onOpenChange = useCallback((open: boolean) => { if (!open) close() }, [close])
  const title = msg('{{provider}} setup guide', { provider: selection?.provider === 'gmail' ? msg('Gmail') : msg('Google Drive') })
  const presentation = useMemo(() => ({ title, available: !!selection?.guide, open: !!selection?.guide,
  ...
  }), [title, selection?.guide, onOpenChange, width, reset, locale, opener])
  useInspectorPresentation(presentation)
  const guide = useInspectorPortal(selection?.guide ? <GoogleRegistrationGuide provider={selection.provider} headingRef={guideHeading}
    onContinue={() => setSelection({ ...selection, guide: false })} /> : null)
  return <>
    <RegistrationRow provider="google-drive" available={available} opener={driveOpener} onOpen={() => { opener.current = driveOpener.current; setSelection({ provider: 'google-drive', guide: false }) }} />
    <RegistrationRow provider="gmail" available={available} opener={gmailOpener} onOpen={() => { opener.current = gmailOpener.current; setSelection({ provider: 'gmail', guide: false }) }} />
    {selection && !selection.guide && <GoogleRegistrationDialog provider={selection.provider} available={available} openerRef={opener} guideFocusRef={guideHeading} onClose={close}
      onGuide={() => setSelection({ ...selection, guide: true })} />}
    {guide}
  </>
}

function RegistrationRow({ provider, available, opener, onOpen }: { provider: ConnectionProvider; available: boolean; opener: RefObject<HTMLButtonElement | null>; onOpen: () => void }) {
  ...
  return <>
    <SettingRow title={title} description={state === undefined ? msg('Loading…') : !usable ? msg('Unavailable') : state === 'setup-required' ? msg('Not configured') : msg('Configured')}
      action={<Button ref={opener} disabled={!usable} aria-label={msg('{{provider}} registration', { provider: title })} onClick={onOpen}>{state === 'setup-required' ? msg('Set up') : msg('Manage')}</Button>} />
  </>
}
