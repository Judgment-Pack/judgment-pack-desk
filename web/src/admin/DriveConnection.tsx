...
import { authorizeDrive, connectionCall, CONNECTIONS_KEY, useDriveStatus } from '../connections/client'

export function DriveConnection({ available }: { available: boolean }) {
 useLocale()
 const query = useDriveStatus(available), client = useQueryClient()
 ...
 async function configure(chosen?: File) {
  ...
  try {
   ...
   await connectionCall('configure', { clientId: data.client_id, clientSecret: data.client_secret })
 ...
 async function connect() {
  ...
  try { await authorizeDrive('connect', controller.signal); await refresh() }
  catch (cause) { if (!controller.signal.aborted) setError((cause as Error).message) }
  finally { operation.current = null; setBusy(false) }
 }
 async function disconnect() {
  setBusy(true); setError('')
  try { const result = await connectionCall<{revoked: boolean}>('disconnect'); await refresh(); setOpen(false); if (!result.revoked) setNotice(msg('Disconnected here. Remove access in your Google account to finish revoking access.')) }
 ...
 return <>
  <SettingRow title={msg('Google Drive')} description={description} action={state === 'unavailable' || state === 'blocked' ? undefined : <Button ref={opener} disabled={busy} onClick={() => { setError(''); setOpen(true) }}>{state === 'setup-required' ? msg('Set up') : msg('Manage')}</Button>} />
  ...
  <Dialog open={open} onOpenChange={next => { if (!busy) setOpen(next) }} title={msg('Google Drive')} openerRef={opener}
   description={msg('Only files you select are used. Google credentials stay with the gateway.')}
   ...
   {state === 'connected' ? <><p>{query.data?.account?.email}</p><p>{msg('Disconnecting does not delete documents already attached to chats.')}</p></> : <p>{msg('Register a Desktop app in Google Cloud and enable the Drive and Picker APIs. Select its downloaded credentials JSON.')}</p>}
   <input ref={file} type="file" accept=".json,application/json" hidden onChange={event => void configure(event.target.files?.[0])} />
   {state !== 'connected' && <p><a href="https://developers.google.com/workspace/drive/picker/guides/desktop-mobile-picker" target="_blank" rel="noreferrer">{msg('Guide')}</a></p>}
