...
import { ConnectionRequestError, authorizeDrive, connectionCall, CONNECTIONS_KEY, useDriveStatus, type ConnectionProvider, type ConnectionStatus, type MailSearch, type MailSelection, type SourceSearch, type SourceSelection } from './client'
...
export function ConnectionsPane({ request, target, onProvider, onClose, onBusy, onAttached = onClose }: {
 ...
 useEffect(() => { setReconnect(false); upload.clearError() }, [provider, context])
 ...
 useEffect(() => {
  if (canAttach && !reconnect && state === 'connected' && (provider === 'gmail' || provider === 'obsidian')) void search()
 }, [canAttach, reconnect, state, provider, status?.data?.account?.id, context])
 ...
 <section ref={panel} tabIndex={-1} className={styles.pane} aria-label={providerName(provider)}>
  <div className={styles.body}>
   ...
   {unavailable ? <p>{state === 'blocked' ? msg('Managed by your organization') : msg('Local processing is unavailable. Check the details in Admin → Storage & data.')}</p> : state === undefined ? <p role="status">{msg('Loading…')}</p> : state !== 'connected' || reconnect ? <>
  ...
  <footer className={styles.footer}>
   ...
   {working && <p role="status">{upload.reading ? systemMessage(upload.progress) : state === 'connected' || provider === 'obsidian' ? msg('Loading…') : provider === 'notion' ? msg('Continue in the Notion sign-in window.') : msg('Continue in the Google sign-in window.')}</p>}
   <div className={styles.actions}>
    {working ? <Button variant="quiet" onClick={() => { cancel(); if (upload.reading) upload.cancel() }}>{msg('Cancel')}</Button> : unavailable ? <Button onClick={() => void status?.refetch()} disabled={!available}>{msg('Retry')}</Button> : reconnect ? <Button variant="primary" disabled={provider === 'obsidian' && !vault.trim()} onClick={() => void connect()}>{msg('Reconnect')}</Button> : state === 'not-connected' ?
