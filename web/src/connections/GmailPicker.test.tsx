...
function view(open = true, onSelect = vi.fn()) {
  return <QueryClientProvider client={testQueryClient()}><GmailPicker open={open} onOpenChange={vi.fn()} state="connected" openerRef={createRef()} onSelect={onSelect} /></QueryClientProvider>
}
it('keeps search metadata out of chat and attaches only checked emails', async () => {
  calls.call.mockImplementation(async method => method === 'search' ? {messages} : [{messageId:'abc1',grant:'aa'.repeat(32)}])
  ...
  expect(calls.call).toHaveBeenCalledWith('select',{messageIds:['abc1']},expect.any(AbortSignal),'gmail')
})
it('limits selection to four and allows deselection', async () => {
  calls.call.mockResolvedValue({messages}); render(view())
...
it('cancels a pending selection when the picker closes', async () => {
  ...
  calls.call.mockImplementation(method => method === 'search' ? Promise.resolve({messages}) : new Promise(resolve => {complete=resolve}))
...
it('uses the submitted query with its pagination token after the input changes', async () => {
  calls.call.mockResolvedValue({messages,nextPageToken:'page-two'}); render(view())
