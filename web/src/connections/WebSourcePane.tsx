...
export function WebSourcePane({ request, target, onClose, onAttached, onBusy }: {
 ...
 const upload = useChatAttachments(store, request.chatId ?? '', running || !available, effective.config.research, request.destination)
 const [url, setURL] = useState(''), [invalid, setInvalid] = useState(false)
 const input = useRef<HTMLInputElement>(null)
 const capacity = Boolean(request.destination ? (request.destination.current()?.length ?? 4) < 4 : chat && (chat.attachments?.length ?? 0) < 4)
 useEffect(() => { input.current?.focus({ preventScroll: true }) }, [target])
 useEffect(() => { if ((!chat && !request.destination) || running) onClose() }, [Boolean(chat), running, onClose])
