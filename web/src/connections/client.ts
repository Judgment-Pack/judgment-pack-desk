...
export function useDriveStatus(enabled = true, provider: ConnectionProvider = 'google-drive') {
 return useQuery({ queryKey: provider === 'google-drive' ? CONNECTIONS_KEY : [...CONNECTIONS_KEY, provider], queryFn: ({ signal }) => connectionCall<ConnectionStatus>('status', {}, signal, provider), enabled, retry: false, staleTime: 30_000 })
}
