...
const statusPatterns = Object.entries(en).filter(([, source]) => source.includes('{{') && !source.includes('<') && (source.includes('{{count}}') || source.replace(/\{\{\w+\}\}/g, '').trim().length >= 12)).map(([key, source]) => {
  const names: string[] = []
...
export function systemMessage(source: string): string {
  ...
  for (const entry of statusPatterns) {
    ...
    if (found) return msg(entry.key, Object.fromEntries(entry.names.map((name, index) => {
      ...
      return [name, name === 'count' && /^\d+$/.test(value) ? Number(value) : value]
    })))
  }
  return source
}
