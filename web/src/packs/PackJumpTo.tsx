...
export function PackJumpTo({ model, at, onJump }: { model: LogicProjection; at: string | null; onJump: (pointer: string) => void }) {
  ...
  return <Popover title="Jump to" trigger={<Button variant="quiet">Jump to</Button>} open={open}
    ...
    <Input type="search" aria-label="Find section or item" placeholder="Find section or item…" value={query}
    ...
    <div className={styles.jumpItems}>{groups.map(group => <section key={group.id} aria-label={group.label}>
      ...
      {group.items.map(item => <InspectionRow key={item.pointer} label={item.label} aria-label={`Jump to: ${item.label}`}
        current={at === item.pointer} onClick={() => { chosen.current = item.pointer; setOpen(false) }} />)}
    </section>)}</div>
    {!groups.length && <p role="status">No matching sections or items.</p>}
  </Popover>
}
