...
export function LogicInspector({ model, at, groupId, onSelect, trace, advanced, mainContent = false, conditionsVisible = true }: {
  ...
  if (at === null) return <div className={styles.details}><h2>Pack details</h2>{advanced}</div>
  ...
  return <div className={styles.details}>
    ...
    {!mainContent && observed && <p className={styles.observation}>Recorded condition: <strong>{observed}</strong></p>}
    {condition !== undefined && (!mainContent || (!conditionsVisible && selected?.group.id !== 'applicability')) && <section className={styles.group}><h3>Condition</h3>
    ...
    {!mainContent && pointer === '/fallbackOutcome' && <section className={styles.group}><h3>{PACK_TERMS.fallbackOutcome.label} <InfoHelp title={PACK_TERMS.fallbackOutcome.label}>{TERM_HELP.fallbackOutcome}</InfoHelp></h3><p>{outcomeLabel(model.document, value)}</p><p className={styles.meta}>A fallback does not itself request a handoff.</p></section>}
    {isRecord(value) && typeof value.description === 'string' && (!mainContent || !['outcomes', 'sources'].includes(selected?.group.id ?? '')) && <Disclosure className={styles.group} title="Author description"><p>{value.description}</p></Disclosure>}
