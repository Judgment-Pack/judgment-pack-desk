...
export function ConfigureAssistant({ open, onOpenChange, openerRef }: { open: boolean; onOpenChange: (open: boolean) => void; openerRef: RefObject<HTMLElement | null> }) {
  ...
  return <Dialog open={open} onOpenChange={next => { if (next) onOpenChange(true); else close() }} title="Configure Assistant" description="Connect a provider and choose a model. Your API key stays on this computer." openerRef={openerRef}>
