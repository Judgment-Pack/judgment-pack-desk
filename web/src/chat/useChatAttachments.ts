...
export function useChatAttachments(store: ChatStore | null, chatId: string, disabled: boolean, config?: ResearchConfig) {
  ...
  return { attachWeb: (url: string) => attachCloud(undefined, undefined, undefined, {url}), attachSource: (provider: SourceProvider, items: SourceSelection[]) => attachCloud(undefined, undefined, {provider, items}), reading, error, progress, isReading, attach, attachDrive: (items?: DriveSelection[]) => attachCloud(undefined, items), attachGmail: (items: MailSelection[]) => attachCloud(items), cancel, prepare }
}
